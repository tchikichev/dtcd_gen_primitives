...
loc = '/home/user/git/datacad-primitives/well/toPlugin'
os.chdir(loc)
...
# %%
# read conf
conf = pd.read_csv('well_primitives.csv')
# conf['compname'] = conf['iconname'].apply(lambda x: x.split('.')[0])
# conf['w'] = 90
# conf['h'] = 90
# conf.to_csv('well_primitives_mod.csv', index=False)
conf.head()
# %%
conf.dropna(subset=['compname','iconname', 'w', 'h'], inplace=True)
conf
#%%
# template json
...
# dict_keys([
# 'single_center', 'single_top', 
# 'pair_vertical', 'pair_vertical_directed', 
# 'pair_vertical_directed_indication', 
# 'pair_horizontal', 
# 'tri_vertical', 'tri_horizontal', 
# 'four_sides', 
# '1c_2cl', '1c_2ul', 
# 'lr', 'lr08', 'l08cr'])
portconfig_json
...
for _,pr in conf.iterrows():
    ...
    compname = pr['compname']
    img_path = os.path.join(loc, 'img', pr['iconname'])
...
# %%
# pr1 = pd.read_csv("./props/oil_dns.csv")
...
# %%
pr1 = pd.read_csv("props/oil_dns.csv", index_col=0)
pr1.drop(['status', 'value', 'input'], axis=1, inplace=True)
# %%
